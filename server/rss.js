RssFeed.publish('jobs', function(query) {
  ...
  self.setValue('title', self.cdata('We Workin PPC - Recent Jobs'));
  self.setValue('description', self.cdata('This is a feed of recent jobs posted to We Work PPC.'));
...
var profileRss = function(query) {
  ...
  self.setValue('title', self.cdata('We Work in PPC - Recent Profiles'));
  self.setValue('description', self.cdata('This is a feed of recent profiles listed on We Workin PPC.'));
  self.setValue('link', Meteor.absoluteUrl());
  self.setValue('lastBuildDate', lastBuildDate);
  self.setValue('pubDate', pubDate);
  self.setValue('ttl', 1);
